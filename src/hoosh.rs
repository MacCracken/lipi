...
pub struct QueryResponse {
    ...
    /// Whether the response was generated by an LLM or from lipi's data.
...
pub enum ResponseSource {
    /// Direct lookup from lipi's data (high confidence).
    LipiData,
    /// Generated by an LLM via hoosh.
    LlmGenerated,
    /// Combination of lipi data + LLM elaboration.
    Hybrid,
}
...
/// Attempt to answer a query using lipi's built-in data (no LLM needed).
///
    ...
    match query {
        LanguageQuery::ExamplesForPhoneme { language, ipa, .. } => {
            ...
            if inv.has(ipa) {
                Some(QueryResponse {
                    query_id: None,
                    source: ResponseSource::LipiData,
                    content: format!(
